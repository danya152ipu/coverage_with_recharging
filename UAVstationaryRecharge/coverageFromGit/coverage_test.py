import numpy as np
from coverageFromGit.coverage_planner import CoveragePlanner, HeuristicType, PlannerStatus
...
def plot_map(target_map, trajectory, map_name="map", params_str=""):
    ...
    cmap = mpl.colors.ListedColormap(
        ['w', 'k', start_position_color, status_color_ref[PlannerStatus.FOUND],
    ...
    ax.arrow(trajectory[0][2] - init_direction[1] / 2, trajectory[0][1] - init_direction[0] / 2, init_direction[1],
             init_direction[0], width=0.1,
...
def getOptimalPath(maska):
        ...
        for orientation in orientations:
            if test_show_each_result:
                print("\n\nIteration[map:{}, cp:{}, initial_orientation:{}]".format(
                    maps[0], heuristic.name, orientation))
    ...
    # Show results
    print("Map tested: {}".format(maps[0]))
    ...
    plot_map(target_map, compare_tb[0][5], map_name=maska,
             params_str="Heuristic:{}, Initial Orientation: {}".format(compare_tb[0][0],
